function [data,tau,sD,TEB]=P5D_Q2D_RS_SIMON(gN, visualize)
...
if visualize
    ...
    [g2D, data02D] = proj(sD.grid,data0,[0 0 1 1 1],'min');
...
sD.dynSys = Unicycle5DRelDubins([0, 0, 0,0,0],acc_max, gamma_max, k1_lim, [k2_min k2_max]);
...
if visualize
    extraArgs.visualize.plotData.plotDims = [1 1 1 0 0];
    extraArgs.visualize.plotData.projpt = [ 0 0];
    ...
    extraArgs.visualize.sliceLevel = 1;
...
if visualize
    ...
    hold on
    subplot(1,2,2)
    [g2D, data2D] = proj(sD.grid, data, [0 0 1 1 1 ], 'min');
    ...
    [g3D, data3D] = proj(sD.grid,data,[0 0 0 1 1],'min');
    [~, data03D] = proj(sD.grid,data0,[0 0 0 1 1],'min');
    [g2D, data2D] = proj(sD.grid,data,[0 0 1 1 1]);%'max');
...
%save
%%
planner_data.k1_min  = -k1_lim;
planner_data.k1_max = -k1_lim;
planner_data.k2_min = k2_min;
planner_data.k2_max = k2_max;
save(['Dubin5D' num2str(gN(1)) '_dt0' num2str(dt*100) '_t' ...
    num2str(tMax) 'converge.mat'], 'TEB','sD', 'data','deriv','planner_data','-v7.3')
    %%
end
